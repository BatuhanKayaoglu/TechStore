...
namespace TechStore.Infrastructure.Persistance.Context
{
    public class SeedData
    {
        private static List<User> GetUser()
        {
            var result = new Faker<User>("tr") // tr dilini kullanarak olustur diyorum.    
        ...
        public async Task SeedAsync(IConfiguration configuration)
        {
            ...
            dbContextBuilder.UseSqlServer(configuration["TechStoreDbConnectionString"]);

            var context = new TechStoreContext(dbContextBuilder.Options);

            var users = GetUser();
            var userIds = users.Select(i => i.Id);

            await context.Users.AddRangeAsync(users); // Gidecek user tablosuna users'ları atacak.
            ...
            var entries = new Faker<Entry>("tr")
                ...
                .RuleFor(i => i.CreatedById, i => i.PickRandom(userIds)) // kimin bu entryi olusturdugunu userId üzerinden rastgele seçeçek.
                .Generate(150);
